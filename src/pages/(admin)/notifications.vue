...
.full-notification-list article {
  display: grid;
...
.full-notification-list article:hover {
  background: var(--kui-color-item-hover);
}
.full-notification-list article.unread {
  background: color-mix(in srgb, var(--kui-color-primary) 5%, transparent);
}
.full-notification-list h3 {
  margin: 0;
  color: var(--kui-color-text-title);
  font-size: 14px;
}
.full-notification-list p {
  margin: 7px 0;
  color: var(--kui-color-text-description);
  line-height: 1.6;
}
.full-notification-list small {
...
@media (max-width: 560px) {
  ...
  .full-notification-list article > .k-btn {
    grid-column: 2;
    justify-self: end;
  }
}
