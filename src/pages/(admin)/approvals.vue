<template>
  <div class="approval-page">
    ...
    <ListPanel
      :summary="`${filteredApprovals.length} 条记录`"
    ...
    >
      ...
      <Table
        ...
        checkable
        :disabled-keys="disabledApprovalKeys"
...
interface ApprovalItem extends TableRecord {
  id: string;
  title: string;
  type: ApprovalType;
  applicant: string;
...
const approvals = ref<ApprovalItem[]>([
  {
    ...
    department: "销售中心",
    amount: 3680,
    createdAt: "2026-08-23 10:42",
    status: "pending",
    description: "上海客户现场方案沟通，包含交通、住宿及市内出行费用。",
  },
  {
    id: "AP-20260823-017",
    title: "研发测试设备采购",
    ...
    department: "研发中心",
    amount: 28600,
    createdAt: "2026-08-23 09:18",
    status: "pending",
    description: "移动端兼容性测试所需设备，共 6 台，已完成三方询价。",
  },
  {
    id: "AP-20260822-096",
    title: "年度框架服务合同",
    ...
    department: "产品中心",
    amount: 128000,
    createdAt: "2026-08-22 16:36",
    status: "pending",
    description: "数据服务年度框架协议，法务已完成条款初审。",
  },
  {
    id: "AP-20260822-082",
    title: "个人年假申请",
    ...
    applicant: "顾言",
    department: "设计中心",
    createdAt: "2026-08-22 14:05",
    status: "approved",
    description: "计划休假 3 个工作日，当前工作已完成交接。",
  },
  {
    id: "AP-20260821-063",
    title: "市场活动物料采购",
    ...
    status: "rejected",
    description: "秋季发布会现场物料及宣传品制作。",
  },
]);
const tableColumns: Column[] = [
  { title: "审批事项", key: "subject", width: 300 },
  { title: "申请人", key: "applicant", width: 100 },
  { title: "部门", key: "department", width: 120 },
  { title: "金额", key: "amount", width: 120 },
  { title: "申请时间", key: "createdAt", width: 165 },
...
const stats = computed(() => [
  {
    label: "待我审批",
    value: approvals.value.filter((item) => item.status === "pending").length,
    color: "#f59e0b",
    icon: Clock,
  },
  {
    label: "今日通过",
    value: approvals.value.filter((item) => item.status === "approved").length,
    color: "#22a06b",
    icon: CircleCheck,
  },
  {
    label: "已驳回",
    value: approvals.value.filter((item) => item.status === "rejected").length,
    color: "#ef6b73",
    icon: CircleX,
  },
  {
...
const approvalFlow = computed(() => {
  ...
  return [
    {
      name: "申请人提交",
      detail: `${selected.value.applicant} · ${selected.value.createdAt}`,
      state: "done",
    },
    {
      name: "直属负责人审批",
...
const flowColor = (state: string) =>
  state === "done"
    ? "var(--kui-color-success)"
    : state === "rejected"
      ? "var(--kui-color-danger)"
      : "var(--kui-color-text-placeholder)";
const openDetail = (record: ApprovalItem) => {
  selected.value = record;
  detailOpen.value = true;
};
const approve = (record: ApprovalItem) => {
...
const approveSelected = () => {
  const selectedIds = new Set(selectedKeys.value);
  ...
  selectedKeys.value = [];
  message.success(`已通过 ${pending.length} 条审批`);
};
const resetFilters = () => {
...
.approval-stat :deep(.k-card-body) {
  display: flex;
  gap: 11px;
  align-items: center;
}
