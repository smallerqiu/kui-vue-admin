        ...
        <GridItem v-for="project in filteredProjects" :key="project.id">
          <Card bordered class="project-card">
            ...
            <Flex justify="space-between" align="center" class="project-footer">
              ...
              <Button size="small" theme="plain" :icon="ArrowRight" @click="openDetail(project)">查看详情</Button>
            </Flex>
          </Card>
        </GridItem>
      </Grid>
    ...
    >
      <template v-if="selectedProject">
        <div class="detail-heading">
          <span class="project-icon" :style="{ color: selectedProject.color, background: `${selectedProject.color}18` }">
            <Icon :type="FolderKanban" />
          </span>
          <div><strong>{{ selectedProject.name }}</strong><small>{{ selectedProject.code }}</small></div>
          <Tag :color="statusMap[selectedProject.status].color" theme="fill">{{ statusMap[selectedProject.status].label }}</Tag>
        </div>
        <p class="detail-description">{{ selectedProject.description }}</p>
        <Grid :cols="2" :x-gap="12" :y-gap="12" class="detail-facts">
          <GridItem><small>负责人</small><strong>{{ selectedProject.owner }}</strong></GridItem>
          <GridItem><small>计划交付</small><strong>{{ selectedProject.deadline || '待确认' }}</strong></GridItem>
          <GridItem><small>项目成员</small><strong>{{ selectedProject.members.length }} 人</strong></GridItem>
          <GridItem><small>当前进度</small><strong>{{ selectedProject.progress }}%</strong></GridItem>
        </Grid>
        <div class="detail-section">
          <Flex justify="space-between"><strong>交付进度</strong><span>{{ selectedProject.progress }}%</span></Flex>
        ...
        <div class="detail-section">
          ...
          <div class="member-list">
            <div v-for="member in selectedProject.members" :key="member">
              ...
              <span>{{ member }}</span><small>{{ member === selectedProject.owner ? '负责人' : '项目成员' }}</small>
            </div>
          </div>
        </div>
        <div class="detail-section">
          <strong>近期里程碑</strong>
          <TimeLine>
            <TimeLineItem v-for="item in projectMilestones" :key="item.title" :icon="item.state === 'done' ? CircleCheck : Clock" :color="item.state === 'done' ? 'var(--kui-color-success)' : 'var(--kui-color-text-placeholder)'" :time="item.date"><strong>{{ item.title }}</strong></TimeLineItem>
          </TimeLine>
        </div>
        <Space class="detail-actions">
          <Button v-if="selectedProject.status !== 'done'" type="primary" @click="advanceProject">推进进度</Button>
          <Button v-if="selectedProject.status === 'paused'" @click="setProjectStatus('active')">恢复项目</Button>
          <Button v-else-if="selectedProject.status !== 'done'" @click="setProjectStatus('paused')">暂停项目</Button>
        </Space>
      </template>
    </Drawer>
  </div>
...
interface MilestoneItem { title: string; date: string; state: "done" | "pending"; }
...
const keyword = ref(""); const status = ref("all"); const drawerOpen = ref(false); const detailOpen = ref(false);
...
const openDetail = (project: ProjectItem) => { selectedProject.value = project; detailOpen.value = true; };
const projectMilestones = computed<MilestoneItem[]>(() => {
  const progress = selectedProject.value?.progress || 0;
  return [
    { title: "需求范围确认", date: "2026-08-08", state: progress >= 20 ? "done" : "pending" },
    { title: "核心功能联调", date: "2026-08-28", state: progress >= 60 ? "done" : "pending" },
    { title: "验收与正式发布", date: selectedProject.value?.deadline || "待确认", state: progress >= 100 ? "done" : "pending" },
  ];
});
const setProjectStatus = (nextStatus: ProjectStatus) => { if (!selectedProject.value) return; selectedProject.value.status = nextStatus; message.success(nextStatus === "paused" ? "项目已暂停" : "项目已恢复"); };
const advanceProject = () => { if (!selectedProject.value) return; selectedProject.value.progress = Math.min(100, selectedProject.value.progress + 10); selectedProject.value.status = selectedProject.value.progress === 100 ? "done" : "active"; message.success(selectedProject.value.progress === 100 ? "项目已完成" : "项目进度已更新"); };
...
.overview-card div { display: flex; flex-direction: column; }.overview-card strong { color: var(--kui-color-text-title); font-size: 22px; }.overview-card small { color: var(--kui-color-text-description); }
...
.detail-heading { display: flex; gap: 12px; align-items: center; }.detail-heading > div { display: flex; min-width: 0; flex: 1; flex-direction: column; }.detail-heading > div strong { color: var(--kui-color-text-title); font-size: 17px; }.detail-heading small,.detail-description,.detail-section span { color: var(--kui-color-text-description); }.detail-description { margin: 18px 0; line-height: 1.7; }
.detail-facts > div { padding: 12px; border-radius: var(--kui-control-radius); background: var(--kui-theme-fill-bg); }.detail-facts small,.detail-facts strong { display: block; }.detail-facts small { margin-bottom: 5px; color: var(--kui-color-text-description); }.detail-facts strong { color: var(--kui-color-text-title); }
.detail-section { padding-top: 20px; margin-top: 20px; border-top: 1px solid var(--kui-color-border); }.detail-section > strong { display: block; margin-bottom: 12px; color: var(--kui-color-text-title); }.member-list { display: grid; gap: 10px; }.member-list > div { display: grid; grid-template-columns: auto 1fr auto; gap: 10px; align-items: center; }.member-list span { color: var(--kui-color-text); }.member-list small { color: var(--kui-color-text-description); }
