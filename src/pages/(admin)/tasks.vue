<template>
  <div class="task-page">
    <PageHeader
      title="任务中心"
      description="按看板推进任务，及时识别阻塞与延期风险。"
    >
      <template #actions
      ...
      <Kanban :columns="kanbanColumns" :data="filteredTasks" @move="moveTask">
        <template #item="{ item: task }">
          <article class="task-card" @click="openEdit(task)">
            ...
            <p>{{ task.project }}</p>
            <div class="task-tags">
              <Tag v-for="tag in task.tags" :key="tag" size="small">{{
                tag
              }}</Tag>
            </div>
            <Flex
            ...
            >
              <span :class="{ overdue: isOverdue(task) }"
                ><Icon :type="Calendar" /> {{ task.deadline }}</span
              >
              <Tooltip :title="task.owner"
                ><Avatar
                  size="small"
                  :style="{ background: avatarColor(task.owner) }"
                  >{{ task.owner.slice(-1) }}</Avatar
                ></Tooltip
              >
            </Flex>
          </article>
          ...
          <GridItem
            ><FormItem label="负责人"
              ><Select
                v-model="form.owner"
                block
                :options="ownerOptions" /></FormItem
          ></GridItem>
          <GridItem
            ><FormItem label="截止日期"
              ><DatePicker v-model="form.deadline" block /></FormItem
          ></GridItem>
          <GridItem
            ><FormItem label="状态"
              ><Select
          ...
          <GridItem
            ><FormItem label="优先级"
              ><Select
                ...
                block
                :options="priorityOptions" /></FormItem
          ></GridItem>
        </Grid>
        <FormItem label="任务标签"
      ...
      <template #footer>
        <Flex justify="space-between">
          ...
          <Space
            ...
            ><Button type="primary" @click="saveTask">保存</Button></Space
          >
        </Flex>
      </template>
    </Drawer>
...
const ownerOptions = ["陈一", "林夏", "周牧", "宋宁", "方远", "顾言"].map(
...
const filteredTasks = computed(() => {
  ...
  return tasks.value.filter(
    (item) =>
      (!query ||
        `${item.title} ${item.project} ${item.owner} ${item.code}`
          .toLowerCase()
          .includes(query)) &&
...
const isOverdue = (task: TaskItem) =>
  task.status !== "done" && task.deadline < "2026-08-23";
...
const moveTask = ({ item, to }: KanbanMoveEvent) => {
  ...
  message.success(
    `任务已移至“${columns.find((entry) => entry.value === to)?.label}”`,
  );
};
</script>
