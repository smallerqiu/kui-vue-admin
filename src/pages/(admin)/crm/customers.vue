<template>
  <div class="pro-list-page">
    <PageHeader title="客户管理" description="统一管理企业客户、跟进状态和客户价值。">
      <template #actions><Button type="primary" :icon="UserRoundPlus" @click="drawerOpen = true">新增客户</Button></template>
    </PageHeader>
    ...
    <ListPanel :summary="`${filteredCustomers.length} 家客户`">
      ...
      <Table :data="filteredCustomers" :columns="columns" row-key="id" :scroll="{ x: 1050 }">
        ...
        <template #status="{ value }"><Badge :status="statusBadge(value)" :text="statusLabel(value)" /></template>
    ...
    <Drawer v-model="drawerOpen" title="新增客户" :width="480" @ok="createCustomer">
      <Form :model="form" layout="vertical">
        ...
        <Grid :cols="2" :x-gap="12">
          <GridItem><FormItem label="所属行业"><Input v-model="form.industry" clearable /></FormItem></GridItem>
          <GridItem><FormItem label="客户等级"><Select v-model="form.level" block :options="levelOptions" /></FormItem></GridItem>
          <GridItem><FormItem label="联系人"><Input v-model="form.contact" clearable /></FormItem></GridItem>
          <GridItem><FormItem label="联系电话"><Input v-model="form.phone" clearable /></FormItem></GridItem>
        </Grid>
...
const router = useRouter(); const customerData = ref([...customers]);
const stats: Array<{ title: string; data: StatNumberItem }> = [
  { title: "客户总数", data: { value: 1286, separator: ",", desc: "较上月", trend: "+8.6%", trendStatus: "success" } },
  { title: "战略客户", data: { value: 86, desc: "贡献 42% 收入" } },
  { title: "本月新增", data: { value: 38, desc: "目标完成率 76%" } },
  { title: "客户年价值", data: { value: 714, prefix: "¥", suffix: "万", desc: "预测年度收入" } },
];
const columns: Column[] = [
  { title: "客户", key: "customer", width: 290 }, { title: "等级", key: "level", width: 110 },
  { title: "状态", key: "status", width: 110 }, { title: "负责人", key: "owner", width: 100 },
  { title: "联系人", key: "contact", width: 100 }, { title: "订单数", key: "orders", width: 90 },
  { title: "年价值", key: "annualValue", width: 130 }, { title: "最近联系", key: "lastContact", width: 130 },
  { title: "操作", key: "action", width: 90, fixed: "right" },
];
const levelOptions = [{ label: "战略客户", value: "strategic" }, { label: "企业客户", value: "enterprise" }, { label: "成长客户", value: "growth" }];
const statusOptions = [{ label: "合作中", value: "active" }, { label: "跟进中", value: "follow" }, { label: "风险关注", value: "risk" }];
const keyword = ref(""); const level = ref<CustomerLevel>(); const status = ref<CustomerStatus>(); const drawerOpen = ref(false);
const form = reactive({ name: "", industry: "", level: "growth" as CustomerLevel, contact: "", phone: "", email: "" });
const filteredCustomers = computed(() => { const query = keyword.value.trim().toLowerCase(); return customerData.value.filter((item) => (!query || `${item.name}${item.contact}${item.city}`.toLowerCase().includes(query)) && (!level.value || item.level === level.value) && (!status.value || item.status === status.value)); });
const levelLabel = (value: string) => levelOptions.find((item) => item.value === value)?.label || value;
const levelColor = (value: string) => ({ strategic: "purple", enterprise: "blue", growth: "green" }[value] || "gray");
const statusLabel = (value: string) => statusOptions.find((item) => item.value === value)?.label || value;
const statusBadge = (value: string): BadgeStatusType => ({ active: "success", follow: "processing", risk: "warning" }[value] || "default") as BadgeStatusType;
...
const createCustomer = () => {
  ...
  customerData.value.unshift({ id: `C${Date.now()}`, ...form, status: "follow", owner: "待分配", city: "-", annualValue: 0, orders: 0, lastContact: "刚刚", color: "#3a95ff" });
  drawerOpen.value = false; message.success("客户已创建");
};
</script>
