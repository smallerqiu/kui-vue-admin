  ...
  <div class="pro-list-page">
    ...
    <ListPanel :summary="`${filteredProducts.length} 件商品`" :selected-count="selectedKeys.length">
      ...
      >
        ...
        <template #price="{ value }"><strong>¥ {{ Number(value).toLocaleString() }}</strong></template>
        <template #stock="{ value }"><span :class="{ 'stock-low': Number(value) < 20 }">{{ value }}</span></template>
    ...
    <Drawer v-model="drawerOpen" :title="editingId ? '编辑商品' : '新增商品'" :width="480" @ok="saveProduct">
      <Form :model="form" layout="vertical">
        <FormItem label="商品名称"><Input v-model="form.name" clearable placeholder="请输入商品名称" /></FormItem>
        <Grid :cols="2" :x-gap="12">
          <GridItem><FormItem label="SKU"><Input v-model="form.sku" clearable /></FormItem></GridItem>
          <GridItem><FormItem label="分类"><Select v-model="form.category" block :options="categoryOptions" /></FormItem></GridItem>
          <GridItem><FormItem label="销售价格"><InputNumber v-model="form.price" :min="0" prefix="¥" /></FormItem></GridItem>
          <GridItem><FormItem label="库存"><InputNumber v-model="form.stock" :min="0" suffix="件" /></FormItem></GridItem>
        </Grid>
...
interface ProductRow extends TableRecord {
  ...
  stock: number; sales: number; enabled: boolean; updatedAt: string; color: string;
}
const columns: Column[] = [
  { title: "商品", key: "product", width: 280 }, { title: "分类", key: "category", width: 120 },
  { title: "价格", key: "price", width: 120 }, { title: "库存", key: "stock", width: 90 },
  { title: "销量", key: "sales", width: 90 }, { title: "状态", key: "status", width: 120 },
  { title: "更新时间", key: "updatedAt", width: 160 }, { title: "操作", key: "action", width: 100, fixed: "right" },
];
const products = ref<ProductRow[]>([
  { id: "P1", name: "企业协作平台专业版", sku: "KUI-PRO-YEAR", category: "软件服务", price: 12800, stock: 999, sales: 328, enabled: true, updatedAt: "2026-08-23 10:20", color: "#3a95ff" },
  { id: "P2", name: "数据分析服务年度版", sku: "DATA-PRO-YEAR", category: "数据服务", price: 28600, stock: 86, sales: 126, enabled: true, updatedAt: "2026-08-23 09:32", color: "#7b61ff" },
  { id: "P3", name: "团队空间扩容包", sku: "SPACE-500G", category: "增值服务", price: 2399, stock: 12, sales: 892, enabled: true, updatedAt: "2026-08-22 18:06", color: "#22a06b" },
  { id: "P4", name: "私有化部署授权", sku: "PRIVATE-DEPLOY", category: "软件服务", price: 68000, stock: 20, sales: 42, enabled: false, updatedAt: "2026-08-20 14:18", color: "#f59e0b" },
]);
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter((item) => (!query || item.name.toLowerCase().includes(query) || item.sku.toLowerCase().includes(query))
...
const saveProduct = () => {
  ...
  else products.value.unshift({ id: `P${Date.now()}`, ...form, sales: 0, updatedAt: "刚刚", color: "#3a95ff" });
  drawerOpen.value = false; message.success("商品信息已保存");
};
</script>
...
.product-cell small { color: var(--kui-color-text-description); }
.stock-low { color: var(--kui-color-warning); font-weight: 600; }
</style>
