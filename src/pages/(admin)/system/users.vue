  ...
  <div class="pro-list-page">
      ...
      <Table :data="pageUsers" :columns="columns" row-key="id" :scroll="{ x: 900 }">
        <template #user="{ record }">
          <Space>
            ...
            <div class="user-cell"><strong>{{ record.name }}</strong><span>{{ record.email }}</span></div>
          </Space>
        </template>
        <template #role="{ value }"><Tag theme="fill">{{ roleLabel(value) }}</Tag></template>
        <template #status="{ value }">
          <Tag :color="value === 'active' ? 'green' : 'gray'">{{ value === "active" ? "正常" : "已停用" }}</Tag>
        </template>
        <template #action="{ record }">
          <Space compact>
            <Tooltip title="查看"><Button size="small" theme="plain" :icon="Eye" /></Tooltip>
            <Tooltip title="编辑"><Button size="small" theme="plain" :icon="Pencil" @click="openEdit(record)" /></Tooltip>
          </Space>
        </template>
      </Table>
    ...
    <Drawer v-model="drawerOpen" :title="editingId ? '编辑用户' : '新增用户'" :width="440" @ok="saveUser">
      <Form :model="form" layout="vertical">
        <FormItem label="姓名" prop="name"><Input v-model="form.name" clearable placeholder="请输入姓名" /></FormItem>
        <FormItem label="邮箱" prop="email"><Input v-model="form.email" clearable placeholder="name@company.com" /></FormItem>
        <FormItem label="角色" prop="role"><Select v-model="form.role" block :options="roleOptions" /></FormItem>
        <FormItem label="状态" prop="status"><Select v-model="form.status" block :options="statusOptions" /></FormItem>
      </Form>
    </Drawer>
  </div>
</template>
...
const columns: Column[] = [
  { title: "用户", key: "user", width: 250 }, { title: "角色", key: "role", width: 110 },
  { title: "部门", key: "department", width: 140 }, { title: "状态", key: "status", width: 100 },
  { title: "最近登录", key: "lastLogin", width: 170 }, { title: "操作", key: "action", width: 100, fixed: "right" },
];
const users = ref<UserRow[]>([
  { id: "U1001", name: "林晓", email: "linxiao@k-ui.cn", role: "admin", status: "active", department: "产品中心", lastLogin: "2026-08-23 09:42", color: "#3a95ff" },
  { id: "U1002", name: "陈默", email: "chenmo@k-ui.cn", role: "developer", status: "active", department: "研发中心", lastLogin: "2026-08-23 09:18", color: "#7b61ff" },
  { id: "U1003", name: "周宁", email: "zhouning@k-ui.cn", role: "operator", status: "active", department: "运营中心", lastLogin: "2026-08-22 18:06", color: "#22a06b" },
  { id: "U1004", name: "王一", email: "wangyi@k-ui.cn", role: "analyst", status: "disabled", department: "数据中心", lastLogin: "2026-08-20 14:31", color: "#f59e0b" },
  { id: "U1005", name: "何安", email: "hean@k-ui.cn", role: "developer", status: "active", department: "研发中心", lastLogin: "2026-08-19 11:20", color: "#ef6c77" },
]);
const statusOptions = [{ label: "正常", value: "active" }, { label: "已停用", value: "disabled" }];
const roleOptions = [
  { label: "管理员", value: "admin" }, { label: "开发者", value: "developer" },
  { label: "运营", value: "operator" }, { label: "分析师", value: "analyst" },
];
const keyword = ref("");
...
const editingId = ref("");
const form = reactive({ name: "", email: "", role: "developer", status: "active" });
const filteredUsers = computed(() => users.value.filter((user) => {
  ...
  return (!query || user.name.toLowerCase().includes(query) || user.email.toLowerCase().includes(query))
    && (!status.value || user.status === status.value) && (!role.value || user.role === role.value);
}));
...
const saveUser = () => {
  ...
  if (editingId.value) {
    ...
    users.value.unshift({ id: `U${Date.now()}`, ...form, department: "待分配", lastLogin: "尚未登录", color: "#3a95ff" });
  }
