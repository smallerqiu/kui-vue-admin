...
const option = computed<ChartOption>(() => {
  ...
  return {
    ...
    legend: { bottom: 0, icon: "circle", textStyle: { color: dark ? "#bfbfbf" : "#595959" } },
    series: [{
      type: "pie", radius: ["48%", "72%"], center: ["50%", "43%"],
      avoidLabelOverlap: true, padAngle: 2,
      itemStyle: { borderRadius: 5, borderColor: dark ? "#1f1f1f" : "#ffffff", borderWidth: 2 },
      label: { show: false }, emphasis: { scale: true, scaleSize: 6 }, data: props.data,
    }],
  };
});
</script>

<style scoped>.donut-chart { width: 100%; height: 290px; }</style>
